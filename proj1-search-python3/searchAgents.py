...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        self.cornerList = {(1,1):0, (1,top):1, (right,1):2, (right,top):3}
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            nsl = [state[1], state[2], state[3], state[4]] #nsl = newStateList
            if (not hitsWall):
                for corner in self.corners:
                    if (nextx,nexty) == corner and not nsl[self.cornerList[corner]]:
                        nsl[self.cornerList[corner]] = True
                successors.append((((nextx, nexty), nsl[0], nsl[1], nsl[2], nsl[3]), action, 1))
...
def cornersHeuristic(state, problem):
    ...
    max2 = 0
    pos = state[0]
    #inadmissible but performs VERY well (ret1*2)
    ...
    if maxcorner2 == (-1,-1):
        return maxdist
    ret2 = max(maxdist2 + abs(maxcorner[0] - maxcorner2[0]) + abs(maxcorner[1] - maxcorner2[1]), maxdist)
    return ret2
...
def foodHeuristic(state, problem):
    ...
    if not 'totalFood' in problem.heuristicInfo:
        problem.heuristicInfo['totalFood'] = len(foodGrid.asList())
    dict = {}
    '''
    for food in foodGrid.asList():
        dist = mazeDistance(food, pos, problem.startingGameState)
        if dist > maxdist:
            maxfood2 = maxfood
            maxdist2 = maxdist
            maxfood = food
    ...
    if maxfood2 == (-1,-1):
        return maxdist
    ret1 = maxdist2 + mazeDistance(maxfood, maxfood2, problem.startingGameState)
    #print (maxfood, maxfood2, pos, ret1)
    return ret1 * len(foodGrid.asList()) / problem.heuristicInfo['totalFood']
    '''
    for food in foodGrid.asList():
